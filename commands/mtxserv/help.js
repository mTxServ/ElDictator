...
/*
const userPerms = mTxServCommand.userPermissions
	? mTxServCommand.userPermissions.map(perm => permissions[perm]).join(', ')
	: 'None';
const clientPerms = mTxServCommand.clientPermissions
	? mTxServCommand.clientPermissions.map(perm => permissions[perm]).join(', ')
	: 'None';
return msg.say(stripIndents`
			__Command **${mTxServCommand.name}**__${mTxServCommand.guildOnly ? ' (Usable only in servers)' : ''}
			${mTxServCommand.description}${mTxServCommand.details ? `\n${mTxServCommand.details}` : ''}
			**Format:** ${mTxServCommand.usage(mTxServCommand.format || '')}
			**Aliases:** ${mTxServCommand.aliases.join(', ') || 'None'}
			**Group:** ${mTxServCommand.group.name} (\`${mTxServCommand.groupID}:${mTxServCommand.memberName}\`)
			**NSFW:** ${mTxServCommand.nsfw ? 'Yes' : 'No'}
			**Permissions You Need:** ${userPerms}
			**Permissions I Need:** ${clientPerms}
		`);
 */